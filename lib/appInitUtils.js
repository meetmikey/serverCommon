...
exports.initApp = function( appName, actions, conf, callback ) {
  ...
  appInitUtils.conf = conf;
  if ( appInitUtils.conf && appInitUtils.conf.nodetime && appInitUtils.conf.nodetime.useNodetime && appInitUtils.conf.nodetime.accountKey ) {
    var fullAppName = appName + ' - ' + environment;
    winston.doInfo('starting nodetime profile', {fullAppName: fullAppName});
    nodetime.profile({
        accountKey: appInitUtils.conf.nodetime.accountKey
      , appName: fullAppName
    });
  }
...
exports.doInitAction = function( action, callback ) {
  ...
  switch( action ) {
    ...
    case appInitUtils.CONNECT_ELASTIC_SEARCH:
      esConnect.init( appInitUtils.conf, callback );
      break;
