...
// function (err, emailText, emailHTML, attachments)
exports.getLikeEmail = function( user, model, type, isPreview, callback ) {
  ...
  var mixpanelPixelURL = emailTemplates.getLikeEmailMixpanelPixelURL( user, model, type );
  if ( isPreview ) {
    mixpanelPixelURL = '';
  }
  ...
  if ( type === 'link' ) {
    ...
    var linkTitle = model.title;
    if ( ! linkTitle ) {
      linkTitle =  model.get('url');
    }

    var linkImageURL = linkUtils.getSignedImageURL( link );
    var linkImage = null;
    if ( linkImageURL ) {
      if ( isPreview ) {
        linkImage = linkImageURL;
      } else {
        linkImage = attachmentName;
        var attachment = {
            fileName: linkTitle
          , filePath: linkImageURL
          , cid: attachmentName
        };
    ...
    templateData['linkImage'] = linkImage;

  } else {
    var attachmentImageURL = attachmentUtils.getSignedImageURL( attachment );
    var attachmentImage = null;
    if ( attachmentImageURL ) {
      if ( isPreview ) {
        attachmentImage = attachmentImageURL;
      } else {
        attachmentImage = attachmentName;
        var attachment = {
            fileName: attachment.filename
          , filePath: attachmentImageURL
          , cid: attachmentName
        };
        attachments.push( attachment );
      }
    ...
    templateData['attachmentImage'] = attachmentImage;
  ...
  callback( null, html, html, attachments );
}
