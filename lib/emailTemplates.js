...
exports.getImapDisabledEmail = function (isOnboarding) {

  return "Hi there, \n\n\
Thanks for signing up for Mikey for GMail!\n\n\
Unfortunately your domain administrator has disabled IMAP access for all email accounts on your domain.\n\n\
In order to use Mikey at this domain, you'd need to contact your admin and have him/her enable access (the relevant how-to is here http://support.google.com/a/bin/answer.py?hl=en&answer=105694).\n\n\
If you have any questions feel free to reach out to me. We would love to have you using Mikey.\n\n\
Best,\n\
Team Mikey"
...
exports.getLinkTemplateData = function( user, link, resourceType, isPreview, callback ) {
  ...
  templateData['linkTitle'] = linkTitle;
  templateData['linkURL'] = link.url;
  templateData['linkFavicon'] = emailTemplates.getFaviconURL( link.url );
  templateData['summary'] = emailTemplates.getAbridgedSummary (link.summary);
  ...
  if ( ! link.image ) {
    ...
    var attachmentName = constants.LIKE_EMAIL_ATTACHMENT_FILENAME;
    templateData['linkImage'] = 'cid:' + attachmentName;
    var imageCloudPath = linkUtils.getImageCloudPath( link );

    //Apparently we don't gzip the full-size images, but we do gzip the thumbnails.
    var useGzip = false;
    if ( link.imageThumbExists ) {
      useGzip = true;
    }

    var getFromAzure = false;
    cloudStorageUtils.getFile( imageCloudPath, useGzip, getFromAzure, function(err, response) {
      if ( err ) {
        callback( err );
...
exports.getAbridgedSummary = function (summary) {
  if (!summary) {
    return "";
  } else {
    var split = summary.split (" ");
    if (split.length >= constants.EMAIL_MAX_SUMMARY_LEN) {
      return split.slice (0, constants.EMAIL_MAX_SUMMARY_LEN-1).join (" ") + "...";
    } else {
      return summary + "...";
    }
  }
