...
exports.doIndexingJob = function ( job, callback ) {
  if (job.type === 'index' && job.indexType === 'resource') {
    indexingHandler.doIndexingResourceJob (job, callback);
  } else if (job.type === 'index' && job.indexType === 'resourceMeta') {
...
exports.doIndexingResourceMetaJob = function ( job, callback ) {
  ...
  var mailBodyRemotePath = conf.aws.s3Folders.rawEmail + '/' + mailId;
  var useGzip = true;
  var inAzure = false;
...
exports.getIndexDataForResource = function (resource, resourceBytes, isLink) {
  ...
  if (!isLink) {
    ...
    if (!resource.isImage && resource.docType != 'video' && resource.docType != 'image') {
