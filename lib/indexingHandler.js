...
exports.updateResourceMetadataIndexState = function( resourceMetadata, isLink, esUtilsError, callback ) {
  ...
  Model.findOneAndUpdate( filter, updateSet, function (err, foundResourceMeta) {
    if ( err ) { 
      ...
      else if (foundResourceMeta.index.attempts > constants.MAX_INDEXING_ATTEMPTS) {
...
exports.updateResourceIndexState = function( resource, isLinkInfo, esUtilsError, callback ) {
  ...
  if ( ! resource ) { callback( winston.makeMissingParamError('resource') ); return; }
  if ( typeof isLinkInfo === 'undefined' ) { callbcak( winston.makeMissingParamError('isLinkInfo') ); return; }
  ...
  Model.update( filter, updateSet, function (err, foundResource) {
    if ( err ) { 
      callback ( winston.makeMongoError(err) );
    } else if ( !foundResource ) {
      callback ( winston.makeError('No records affected when updating indexState for resourceMeta', filter) );
      ...
      else if (foundResource.index.attempts > constants.MAX_INDEXING_ATTEMPTS) {
        esUtilsError.deleteFromQueue = true;
 
        //TODO: index as a stub resource?
        callback (winston.makeError ('failed indexing and not deleting from queue', esUtilsError));
      }
      else {
