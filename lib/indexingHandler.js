...
exports.pushJobToQueue = function ( isQuick, job, isReindexing, callback ) {
  ...
  queueAddFunction (job, function( err ) {
    if ( err ) {
      callback( winston.makeError('Could not add job to worker queue', {job : job, isReindexing: isReindexing}) );
...
exports.createIndexingJobForDocument = function ( isQuick, model, isLink, isReindexing, callback) {
  ...
  if ( ! job ) {
    ...
    indexingHandler.pushJobToQueue( isQuick, job, isReindexing, callback );
  }
}

exports.createDeleteJobForDocument = function (userId, modelId, modelName, callback) {
  var job = {
    jobType : 'deleteIndex',
    resourceId : modelId,
    userId : userId,
  ...
  indexingHandler.pushJobToQueue (true, job, false, callback);
}
...
exports.doDeleteFromIndexJob = function (job, callback) {
  var indexName = job.userId + '_' + job.indexName;
  esUtils.delete ( indexName, job.indexType, job.modelId, null, function (err) {
    if (err) {
      callback (err);
    } else {
      var Model;
      if (job.modelName == 'Attachment') {
        Model = AttachmentModel;
      } else {
        Model = LinkModel;
      }

      Model.findById (job.modelId, function (err, foundModel) {
        foundModel.index = [];
...
exports.doIndexingDocumentJob = function ( job, callback ) {
  ...
  AttOrLinkModel.findById (modelId, function (err, foundAttOrLink) {
    if (err) {
      ...
      // get the lock
      indexingHandler.acquireLockAfterTimeout (AttOrLinkModel, foundAttOrLink._id, 
        0, function (err, lockedAttOrLink) {

        if (err) {
          callback (err);
        } else {

          indexingHandler.getMailAndResource (isLink, mailId, mailPath, resourcePath, lockedAttOrLink.fileSize, lockedAttOrLink.docType, lockedAttOrLink.index, function (err, response) {
            if (err) {
              callback (err);
            } else {
              var mailObj = response [0];
              var resourceBuffer = response [1];
              var asStub = !resourceBuffer || resourceBuffer === '';

              var indexData = indexingHandler.getIndexDataForDocument( lockedAttOrLink, resourceBuffer, mailObj, isLink );
              var parentId = null;

              esUtils.index( esUtils.getIndexAliasForUser (lockedAttOrLink.userId), 'document', modelId, indexData, parentId,
                function( esUtilsError ) {
                  if (esUtilsError && esUtils.isHardFail (esUtilsError.message)) {
                    ...
                    indexingHandler.indexResourceAsStub (lockedAttOrLink, modelId, mailObj, isLink, callback);
                  } else {
                    indexingHandler.updateAttOrLinkIndexState( lockedAttOrLink, isLink, esUtilsError, asStub, callback );
                  }
                }
              );
...
exports.acquireLockAfterTimeout = function (AttOrLinkModel, modelId, timeout, callback) {

  winston.doInfo ('acquireLockAfterTimeout', {timeout: timeout});

  // indexLockTS + timeout < now or indexLockTS < now - timeout
  var filter = {
      '_id': modelId
    , $or : [ 
      { 'indexLockTS' : {$lt : utils.convertToISOString (Date.now(), -constants.INDEX_LOCK_TIMEOUT)} }, 
      { 'indexLockTS' : {$exists : false} } ]
  };

  setTimeout (function () {

    AttOrLinkModel.findOneAndUpdate (filter, {$set : {indexLockTS : Date.now()}}, function (err, foundAttOrLink) {
      if (err) {
        callback (winston.makeMongoError (err));
      } else if (!foundAttOrLink) {
        winston.doWarn ('indexingHandler: acquireLockAfterTimeout: no unlocked model found', {filter : filter});
        indexingHandler.acquireLockAfterTimeout (AttOrLinkModel, modelId, timeout + constants.INDEX_HANDLER_LOCK_CHECK_TIMEOUT, callback);
      } else {
        callback (null, foundAttOrLink);
      }
    });
...
exports.getMailAndResource = function (isLink, mailId, mailPath, resourcePath, fileSize, docType, currentIndexState, callback) {
  var useGzip = true;
  var inAzure = false;
  ...
  async.parallel([
    function(asyncCb){
      ...
      cloudStorageUtils.getFile (mailPath, useGzip, inAzure, function (err, res) {
        if ( err ) {
          if ( err && ( winston.getErrorType( err ) == constants.ERROR_TYPE_404 ) ) {
    ...
    function(asyncCb){
      ...
      if (!isLink && !attachmentUtils.resourceDocTypeIndexable (docType, fileSize)) {
        asyncCb (null, null);
        ...
        cloudStorageUtils.getFile (resourcePath, useGzip, inAzure, function (err, res) {
          if ( err ) {
            ...
            var capBuffer = isLink;

            if (utils.isWebResponseTooBig (res)) {
              asyncCb (null, '');
            
            } else {

              // convert response to buffer
              utils.streamToBuffer (res, capBuffer, function (err, buffer, isAborted) {
                if (err) {
                  asyncCb (winston.makeError ('could not convert stream to buffer', {err : err}));
                } else if (isAborted) {
                  asyncCb (null, null);
...
exports.updateAttOrLinkIndexState = function( attOrLink, isLink, esUtilsError, asStub, callback ) {
  ...
  // release lock
  attOrLink.indexLockTS = undefined;
