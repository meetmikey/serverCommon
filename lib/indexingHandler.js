...
exports.createIndexingJobForResource = function ( model, isLinkInfo, callback ) {
  ...
  var job = {
    jobType : 'index',
    indexName : conf.elasticSearch.indexName,
    indexType : 'resource',
...
exports.doIndexingResourceJob = function ( job, callback ) {
  ...
  if (isLinkInfo) {
    cloudPath = conf.aws.s3Folders.linkInfo + '/' + modelId;
  } else {
    cloudPath = conf.aws.s3Folders.attachment + '/' + modelId;    
  }
          ...
          try {
            ...
            Model.findById (modelId, function (err, foundModel) {
              if (err) {
                ...
                indexingHandler.indexResourceMetadata( foundModel, mail, parentId, isLink );
              }
            });
  ...
  esUtils.index( esConnect.indexName, 'resource', resourceId, indexData, parentId,
    function( esUtilsError ) {
      if ( isLinkInfo ) {
        ...
        indexingHandler.updateResourceIndexState( resource, isLinkInfo, esUtilsError );
...
exports.indexResourceMetadata = function( resourceMetadata, mail, parentId, isLink ) {
  ...
  esUtils.index( esConnect.indexName, 'resourceMeta', String(resourceMetadata._id), indexData, parentId,
    function( esUtilsError ) {
      indexingHandler.updateResourceMetadataIndexState( resourceMetadata, isLink, esUtilsError );
    }
  );
}


exports.updateResourceMetadataIndexState = function( resourceMetadata, isLink, esUtilsError ) {
  ...
  Model.update( filter, updateSet, indexingHandler.updateIndexStateCallback );
}

exports.updateResourceIndexState = function( resource, isLinkInfo, esUtilsError ) {
