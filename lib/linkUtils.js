...
exports.isValidTopLevelDomain = function( url ) {
  ...
  linkUtils.getValidTopLevelDomains( function( err, topLevelDomains ) {
    if ( err ) {
...
exports.getValidTopLevelDomains = function( callback ) {

  if ( linkUtils.validTopLevelDomains === null ) {
    linkUtils.validTopLevelDomains = [];
    fs.readFile( conf.validTopLevelDomainsFile, 'utf8', function(err, data) {
      if ( err ) {
        winston.doError('error reading top level domains file');
      ...
      callback( null, linkUtils.validTopLevelDomains );
    });
...
exports.getLinkInfoAndUpdateLink = function( link, mail, callback ) {
  ...
  linkUtils.getLinkInfo( link, mail, function( err, linkInfo ) {
    if ( err ) {
      callback( err );
