...
exports.checkAndMarkLinkPromoted = function( link, contactData, isSentByUser, isLinkAlreadyInDB, callback ) {
  ...
  } else if ( url && url.length > constants.MAX_URL_LENGTH ) {
    winston.doWarn('linkUtils: checkAndMarkLinkPromoted: url is not promotable because of length', {url: url});
    link.nonPromotableReason = 'length';
    callback();
...
exports.getNumDuplicateLinksForUser = function( url, userId, callback ) {
  ...
  LinkModel.count( filter, function(err, numDuplicateLinks) {
    if ( err ) {
...
exports.getSignedImageURL = function( link ) {
  var url = '';
  if ( ! link ) {
    return url;
    ...
    var expireTime = constants.DEFAULT_SIGNED_URL_EXPIRE_TIME_MINUTES;
    if ( expireTimeMinutesInput ) {
      expireTimeMinutes = expireTimeMinutesInput;
    }
    url = cloudStorageUtils.signedURL( imageCloudPath, expireTimeMinutes, link );
    return url;
  }
}