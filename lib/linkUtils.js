      ...
      linkUtils.updateLinkFromLinkInfo( link, linkInfo, function( err, updatedLink ) {
        if ( err ) {
          ...
          if ( link.isPromoted && link.isFollowed ) {
            indexingHandler.createIndexingJobForDocument(message.isQuick, link, true, false, callback );
    ...
    if ( err ) {
      ...
      LinkInfoModel.findOne({comparableURLHash: comparableURLHash}, function(err, linkInfo) {
        if ( err ) {
          ...
          if ( ( ! previousLinkInfo._id ) || ( ! linkInfo.followJobCreated ) ) {
            //we either just created this linkInfo, or we hadn't previously created the link follow job
