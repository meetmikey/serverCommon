...
exports.checkAndMarkLinkPromoted = function( link, contactData, isSentByUser, isLinkAlreadyInDB, callback ) {
  ...
  } else if ( linkHandler.isBadContactRatio( contactData ) ) {
    //winston.doInfo('linkHandler: isPromotableWithData: url is not promotable because contact ratio is too high');
    callback( null, false, 'senderRatio' );
...
exports.isBadContactRatio = function( contactData ) {
  ...
  if ( denominator == 0 ) {
    winston.doWarn('linkHandler: isBadContactRatio: zero denominator');
    return false;
  }
