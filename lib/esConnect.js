...
exports.init = function( conf, callback ) {
  ...
  esConnect.client = new ElasticSearchClient(serverOptions);

  console.log (esConnect.client);
...
exports.checkHealth = function(callback) {
  esConnect.client.health()
    .on('data', function(data) {
      ...
      var minStatus = 'yellow';

      if (parsed && parsed.status && parsed.status != minStatus) {
...
exports.checkMappings = function (callback) {
  ...
  async.parallel ([
    function (cb) {
      esConnect.client.getMapping (esConnect.conf.elasticSearch.indexName, 'resource')
        .on('data', function (data) {
          var parsed = JSON.parse (data);
    ...
    function (cb) {
      esConnect.client.getMapping (esConnect.conf.elasticSearch.indexName, 'resourceMeta')
        .on('data', function (data) {
          var parsed = JSON.parse (data);
