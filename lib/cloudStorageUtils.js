...
exports.putBuffer = function(buffer, cloudPath, headers, useGzip, putInAzure, callback) {
  ...
  if ( useAzure ) {
    ...
    if ( useGzip ) {
      s3Utils.putGzipBuffer( buffer, cloudPath, headers, 0, callback );
    } else {
      s3Utils.putBuffer( buffer, cloudPath, headers, 0, callback );
    }
  }
}
    ...
    fs.exists(dir + subDir, function (exists) {
      if (exists) {
        ...
        fs.mkdir (dir  + subDir, function (err) {
          if (err) {
            winston.doError ("Error: could not make directory", {path : dir  + subDir});
          }
        });
      }
    });
