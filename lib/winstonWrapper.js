...
winston.logToFiles = function( processName ) {
  if ( ! processName ) {
    winston.doMissingParamError('processName');
    return;
  }
  
  var logDir = conf.logDir + '/' + processName + '/';
  try {
    fs.mkdirSync( logDir );
  } catch ( exception ) { }

  winston.add(winston.transports.File, { filename: logDir + 'all.log', json: false});
  //winston.add(winston.transports.File, { filename: logDir + 'all.log', level: 'warn' });
  //winston.add(winston.transports.File, { filename: logDir + 'all.log', level: 'error' });
  //winston.add(winston.transports.File, { filename: logDir + 'error.log', level: 'error' });
}
...
winston.doError = function(log, extraInput) {
  winston.handleError( winston.makeError(log, extraInput, 5) );
}
...
winston.makeMissingParamError = function(paramName, skipStacktraceLinesInput) {
  ...
  var error = winston.makeResponseError('missing param: ' + paramName, 500, 'internal error', null, skipStacktraceLines);
  return error;
}
