...
winston.makeRequestError = function(requestError, extraInput, skipStacktraceLinesInput) {
  ...
  var extra = winston.mergeExtra( extraInput, {requestError: requestError});
...
winston.makeS3Error = function(s3Err, extraInput, skipStacktraceLinesInput) {
  ...
  var extra = winston.mergeExtra( extraInput, {s3Err: s3Err});
...
winston.makeElasticSearchError = function(esErr, extraInput, skipStacktraceLinesInput) {
  ...
  var extra = winston.mergeExtra( extraInput, {esErr: esErr});
...
winston.makeMissingParamError = function(paramName, skipStacktraceLinesInput) {
  ...
  var error = winston.makeResponseError('missing param: ' + paramName, 500, 'internal error', skipStacktraceLines);
  return error;
}
...
winston.logBreak = function() {
  console.log( constants.LOG_BREAK );
  console.error( constants.LOG_BREAK );
}
