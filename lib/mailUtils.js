...
exports.isAttachmentImage = function( attachment ) {
  ...
  return mailUtils.isContentTypeCandidate( attachment.contentType, 'image' );
}
...
exports.getDocType = function (contentType) {
  ...
  else if (mailUtils.isContentTypeCandidate(contentType, 'image')) {
    return 'image';
  }
  else if (mailUtils.isContentTypeCandidate(contentType, 'pdf')) {
    return 'pdf';
  }
  else if (mailUtils.isContentTypeCandidate(contentType, 'document')) {
    return 'document';
  }
  else if (mailUtils.isContentTypeCandidate(contentType, 'spreadsheet')) {
    return 'spreadsheet';
  }
  else if (mailUtils.isContentTypeCandidate(contentType, 'presentation')) {
    return 'presentation';
  }
  else if (mailUtils.isContentTypeCandidate(contentType, 'music')) {
    return 'music';
  }
  else if (mailUtils.isContentTypeCandidate(contentType, 'video')) {
    return 'video';
  }
  else if (mailUtils.isContentTypeCandidate(contentType, 'archive')) {
    return 'archive';
  }
  else if (mailUtils.isContentTypeCandidate(contentType, 'code')) {
    return 'code';
  }
  else {
    return 'other';
  ...
  if ( mail.indexable || uploadAll ) {
    ...
    cloudStorageUtils.putBuffer( buffer, cloudPath, options, useGzip, useAzure, function (err) {
      if (err) {
        ...
        MailModel.update (query, {$set : {bodyInS3 : 'fail'}}, function (mongoErr, num) {
          if (mongoErr) { 
        ...
        MailModel.update (query, {$set : {bodyInS3 : 'success'}}, function (mongoErr, num) {
          if (mongoErr) { 
            winston.doMongoError(mongoErr, {msg: 'could not update model bodyInS3'});
          }
          else if (num == 0) {
...
exports.downloadMailAndRunParser = function( mailS3Path, mailParser, inAzure, callback ) {
  ...
  cloudStorageUtils.getFile( mailS3Path, false, inAzure, function(err, res) {
    if ( err ) {
      ...
      callback (winston.makeS3Error ('Non-200 status code for download and run mail parser'), {statusCode : res.statusCode, mailS3Path : mailS3Path});
      cloudStorageUtils.printResponse (res, inAzure);
    } else if (inAzure && res && res.properties && !res.properties.blobType) {
      callback (winston.makeAzureError ('Block blob for mail object must not exist', {mailS3Path : mailS3Path}));
      cloudStorageUtils.printResponse (res, inAzure);
    }
    else {
      var calledBack = false;
      winston.doInfo('getFile returned', {mailS3Path: mailS3Path});
      res.on('data', function(data) {
        winston.doInfo('getFile data', {mailS3Path: mailS3Path, length: data.length});
        // data - but data could be something like...
        mailParser.write(data);
      });
      res.on('end', function() {
        winston.doInfo('getFile end', {mailS3Path: mailS3Path});
        if ( calledBack ) {
          winston.doError('BAD! In "res.end", but we already called back', {mailS3Path: mailS3Path});
        } else {
          calledBack = true;
          mailParser.end();
        }
      ...
      res.on('error', function (err) {
        winston.doInfo('getFile error', {mailS3Path: mailS3Path});
        if ( calledBack ) {
          winston.doError('BAD! In "res.error", but we already called back', {mailS3Path: mailS3Path});
        } else {
          calledBack = true;
