...
exports.getLinkInfoFromJob = function( job, callback ) {
  ...
  LinkInfoModel.findById( job.linkInfoId, function(err, linkInfo) {
    if ( err ) {
      ...
      callback( winston.makeError('followLink job: no linkInfo with id. invalid job', 
        {linkInfoId: job.linkInfoId, deleteFromQueue : true}) );
...
exports.followLinkSaveUploadIndexAndUpdateLinks = function( message, linkInfo, userId, callback ) {
  ...
  // even though we checked before creating the follow job the blacklist should be checked again
  // in case we added something manually after a bunch of jobs were already created
  if (utils.containsSubstringFromArray( linkInfo.rawURL, constants.URL_FILTER_TEXT )) {
    ...
    followLinkUtils.followLinkSaveAndUpload( message, linkInfo, userId, function( err ) {
      if ( err ) {
        callback( err );
...
exports.followLinkSaveAndUpload = function( message, linkInfo, userId, callback ) {
  ...
  followLinkUtils.checkAndFollowLink( message, linkInfo, userId, function( err, linkFollowData, mimeType ) {
    ...
    } else if (linkInfo.followType === 'stub') {

      // we don't need to upload the linkinfo data to the cloud since it's a stub
      // meaning that the link exists, but we didn't grab the text behind it
      // since the size of the response was too big!
      followLinkUtils.updateLinkInfoAfterFollowingLink( linkInfo, callback);

    } else if ( ! linkFollowData ) {
      // this case should not be reached but we put it in as a safety check
      winston.doWarn ('checkAndFollowLink returned empty linkFollowData but not as stub or fail followType', 
  ...
  followLinkUtils.checkURLDomain( url, function(err, urlDomainIsOK) {
    if ( err ) {
      ...
      followLinkUtils.followLink( message, linkInfo, userId, function( followLinkErr, linkFollowData, mimeType ) {
        var isSuccess = !followLinkErr && linkInfo.followType !== 'fail';
...
exports.followLink = function( message, linkInfo, userId, callback ) {
  ...
  if ( urlUtils.isPDF(url) ) {
  ...
  } else if (urlUtils.isDropboxFile (url)) {
    ...
    // a normal url we send to diffbot
    ...
    diffbot.article( diffbotData, function(err, response) {
      if ( err || ( ! response ) || ( response.errorCode ) ) {
        ...
        if ( response ) {
          ...
          if (response.errorCode === 429 || response.errorCode === 401) {
            sesUtils.sendInternalNotificationEmail('diffbot token is no longer valid or max api calls exceeded',
              'dangerous diffbot status code received: ' + response.errorCode, function () { });
          }
...
exports.followPDFLink = function( linkInfo, callback ) {
  ...
  webUtils.webGet( url, true, function( err, responseBuffer, resolvedURL, responseHeaders, isAborted) {
    ...
    if ( err ) {
      ...
      linkInfo.followType = 'stub';
      linkInfo.abortedWebGet = true;
      callback();

    } else if ( !responseBuffer ) {
      winston.doWarn('followLinkUtils: followPDFLink: empty response', {url: url, err: err});
      callback();
...
exports.followGoogleDocLink = function( message, linkInfo, userId, callback ) {
  ...
  googleUtils.getAccessToken( userId, function(err, accessToken) {
    if ( err ) {
      if (winston.getErrorType (err) === 'invalid_grant') {
        linkInfo.failReason = 'invalidToken';
        callback(err);
      } else {
        callback(err);
      }
      ...
      webUtils.webGet( docMetadataURL, true, function( err, responseBuffer, resolvedURL, responseHeaders ) {
        if ( err ) {
          winston.doWarn('followLinkUtils: followGoogleDocLink: failed getting metadata', 
            {err: err, linkInfoId: linkInfo._id});
          callback();

        } else if ( ! responseBuffer ) {
          winston.doWarn('followLinkUtils: followGoogleDocLink: no responseBuffer for metadata', 
            {linkInfoId: linkInfo._id});
          callback();
...
exports.getGoogleDocImageTitleSummaryAndData = function( message, linkInfo, docMetadataRaw, accessToken, callback ) {
  ...
  try {
    ...
    callback( winston.makeError('failure parsing google doc metadata', 
      {message: e.message, stack : e.stack, raw : docMetadataRaw}) );
    return;
  }
  ...
  followLinkUtils.addImageToLinkInfo( message, linkInfo, thumbnailLink, function(err) {
    ...
    if ( ! exportLink ) {
      ...
      webUtils.webGet( exportLink, true, function(err, responseBuffer, resolvedURL, responseHeaders, isAborted) {
        if ( err ) {
          callback( err );
...
exports.followLinkDirectly = function(linkInfo, callback) {
  ...
  webUtils.webGet( url, true, function( err, responseBuffer, resolvedURL, responseHeaders, isAborted ) {
    ...
    if ( err ) {
      ...
      var acceptableContentTypes = ['document', 'spreadsheet', 'presentation', 'pdf'];
      ...
      if ( contentType && ( contentType.indexOf ('text/html') != -1 ) ) {
        ...
        callback( null, html, mimeType );
          ...
          acceptableContentTypes.indexOf(attachmentUtils.getDocType (contentType)) != -1 ) {

        var data = responseBuffer;
        var mimeType = contentType;
        linkInfo.followType = 'direct';
        callback (null, data, mimeType);
      
      } else {
        winston.doWarn('followLinkUtils: followLinkDirectly: response not an accetable contentType', 
...
exports.updateLinkInfoAfterFollowingLink = function( linkInfo, callback ) {
  ...
  if ( linkInfo.abortedWebGet ) {
    updateSet['$set']['abortedWebGet'] = linkInfo.abortedWebGet;
  }
...
exports.getRealURL = function(linkInfo) {
  var realURL = '';
  if ( ! linkInfo ) {
    winston.doError('followLinkUtils: getRealURL: no linkInfo!');
    return realURL;
  }
