...
exports.addImageToLinkInfo = function( linkInfo, imageURL, callback ) {
  ...
  cloudStorageUtils.downloadAndSaveStaticImage( imageURL, null, function ( err ) {
    if ( err ) {
      callback( err );

    } else {
      //False here means that it's not in azure.  If we ever come back to find a problem with this, it was Sagar's idea.
      var imagePath = cloudStorageUtils.getImageCloudPathFromURL( imageURL, false );
