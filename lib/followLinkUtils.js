...
exports.createFollowLinkJob = function( linkInfo, userId ) {
  ...
  if ( ( ! job ) || ( job.jobType != 'followLink' ) ) {
    ...
    followLinkUtils.getLinkInfoFromJob( job, function(err, linkInfo) {
      if ( err ) {
        ...
        followLinkUtils.followLinkSaveUploadIndexAndUpdateLinks( linkInfo, userId, callback );
      }
    });
  }
}
...
exports.getLinkInfoFromJob = function( job, callback ) {
  ...
  if ( ! job ) { callback( winston.doMissingParamError('job') ); return; }
  if ( ! job.linkInfoId ) { callback( winston.doMissingParamError('job.linkInfoId') ); return; }
...
exports.followLinkSaveUploadIndexAndUpdateLinks = function( linkInfo, userId, callback ) {
  followLinkUtils.followLinkSaveAndUpload( linkInfo, userId, function( err, linkFollowData ) {
    if ( err ) {
      callback( err );
      ...
      indexingHandler.createIndexingJobForResource( linkInfo, true, function( err ) {
        if ( err ) {
          callback( err );
        } else {
          followLinkUtils.updateAndIndexLinks( linkInfo, callback );
        }
      });
    }
  });
...
exports.followLinkSaveAndUpload = function( linkInfo, userId, callback ) {
  ...
  followLinkUtils.followLink( linkInfo, userId, function( err, linkFollowData, mimeType ) {
    if ( err ) {
      callback( err );

    } else if ( ! linkFollowData ) {
      var warnData = {followType: linkInfo.followType, url: linkInfo.rawURL};
      ...
      winston.doWarn('followLinkUtils: followLinkSaveAndUpload: following failed', {url: linkInfo.rawURL});
...
exports.followLink = function( linkInfo, userId, callback ) {
  ...
  if ( urlUtils.isPDF(url) ) {
    ...
    followLinkUtils.followGoogleDocLink( linkInfo, userId, callback );

  } else { // a normal url we send to diffbot
    followLinkUtils.followDiffbotLink(linkInfo, callback );
  }
}
  ...
  try {
    diffbot.article( diffbotData, function(err, response) {
      ...
      if ( err || ( ! response ) || ( response.errorCode ) ) {
        ...
        followLinkUtils.processDiffbotResponse( response, linkInfo, callback );
      }
    });
  } catch ( diffbotError ) {
...
exports.processDiffbotResponse = function(diffbotResponse, linkInfo, callback) {
  ...
  followLinkUtils.addImageToLinkInfo( linkInfo, imageURL, function(err) {
    if ( err ) {
 ...
 if ( ! linkInfo ) { callback( winston.makeMissingParamError('linkInfo') ); return; }
  ...
  webUtils.webGet( url, function( err, response ) {
      ...
      if ( error || ( ! response ) || ( ! response.body ) ) {
        winston.doWarn('followLinkUtils: followPDFLink: error downloading pdf', {url: url});
        callback();
...
exports.followGoogleDocLink = function( linkInfo, userId, callback ) {
  ...
  googleUtils.getAccessToken( userId, function(err, accessToken) {
    if ( err ) {
      ...
      webUtils.webGet( docMetadataURL, function( err, response ) {
        if ( err ) {
          ...
          winston.doWarn('followLinkUtils: followGoogleDocLink: no body', warnData);
          callback();

        } else {
          var docMetadata = response.body;
          followLinkUtils.getGoogleDocImageTitleSummaryAndData( linkInfo, docMetadata, accessToken,
...
exports.getGoogleDocImageTitleSummaryAndData = function( linkInfo, docMetadataRaw, accessToken, callback ) {
  ...
  followLinkUtils.addImageToLinkInfo( linkInfo, thumbnailLink, function(err) {
    if ( err ) {
      winston.handleError (err);
    ...
    if ( ! exportLink ) {
      ...
      webUtils.webGet( exportLink, function(err, response ) {
        if ( err ) {
          callback( winston.makeError('export webGet error', {err: err.toString(), linkInfoId: linkInfo._id}) );

        } else if ( ( ! response ) || ( ! response.body ) ) {
          winston.doWarn('followLinkUtils: getGoogleDocTitleSummaryAndData: no body', {linkInfoId: linkInfo._id});
          callback( null, null, mimeType );

        } else {
          var docData = response.body;
...
exports.followLinkDirectly = function(linkInfo, callback) {
 ...
 if ( ! linkInfo ) { callback( winston.makeMissingParamError('linkInfo') ); return; }
  ...
  webUtils.webGet( url, function( err, response, resolvedURL ) {
    ...
    if ( err ) {
      winston.doWarn( 'followLinkUtils: followLinkDirectly: error response from webGet', warnData );
      callback();
      
    } else if ( ! response ) {
      winston.doWarn( 'followLinkUtils: followLinkDirectly: no response from webGet', warnData );
      callback();

    } else if ( ! response.body ) {
      winston.doWarn( 'followLinkUtils: followLinkDirectly: no response body from webGet', warnData );
      callback();
      ...
      if (response.headers 
        ...
        winston.doWarn( 'followLinkUtils: followLinkDirectly: response content not an html page or pdf', 
...
exports.addImageToLinkInfo = function( linkInfo, imageURL, callback ) {
  ...
  cloudStorageUtils.downloadAndSaveStaticImage( imageURL, null, function ( err, imagePath ) {
    if ( err ) {
      callback( err );
...
exports.updateAndIndexLinks = function( linkInfo, callback ) {
  ...
  var updateSet = {$set: {
      linkInfoId: linkInfo._id
    , isPromoted: true //Sagar and Justin decided that we can promote all links when following has succeeded (4/15/2013)
  }};
      ...
      LinkModel.find( filter, function( err, foundLinks ) {
        if ( err ) {
          ...
          async.each( foundLinks, function( foundLink, eachCallback ) {
            indexingHandler.createIndexingJobForResourceMeta( foundLink, true, eachCallback );
          }, callback );
        }
      });
    }
