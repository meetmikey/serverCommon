    ...
    if ( err ) {
      ...
      webUtils.webGet( docMetadataURL, function( err, response ) {
        if ( err ) {
          ...
          winston.doWarn('followLinkUtils: followGoogleDocLink: no response', warnData);
          callback();

        } else {
          utils.streamToBuffer( response, function(err, responseBody) {
            if ( err ) {
              callback(err);

            } else if ( ! responseBody ) {
              var warnData = {linkInfoId: linkInfo._id};
              winston.doWarn('followLinkUtils: followGoogleDocLink: no responseBody', warnData);
              callback();

            } else {
              var docMetadata = responseBody;
