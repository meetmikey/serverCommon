...
//callback has arguments: err, userChanged, numNewDays
exports.updateUserAccountStatus = function( user, forceResumeDownloadJob, callback ) {
	...
	upgradeUtils.updateUserIsPremium( user, function(err, isPremiumChanged) {
      ...
      var daysLimitBefore = user.daysLimit;
          ...
          if ( isPremiumChanged || ( ( ! user.isPremium ) && daysLimitChanged ) ) {
            userChanged = true;
          }
          
          if ( forceResumeDownloadJob || userChanged ) {
            var startingPoint;
            mailDownloadUtils.createResumeDownloadNow( user, startingPoint, false, function(err) {
              if ( err ) {
                callback( err );

              } else {
                var daysLimitAfter = user.daysLimit;
                var numNewDays = daysLimitAfter - daysLimitBefore;
                callback( null, userChanged, numNewDays );
              }
            });
    ...
    if ( err ) {
      ...
      ExtraDaysModel.find({userId: userId}, function(err, foundExtraDays) {
        if ( err ) {
          ...
          userUtils.getUserActionEarnedDays( user, function(err, userActionEarnedDays) {
            if ( err ) {
              callback( err );

            } else {
              user.daysLimit = upgradeUtils.calculateDaysLimit( user, foundReferrals, foundExtraDays, userActionEarnedDays );
...
exports.calculateDaysLimit = function( user, foundReferrals, foundExtraDays, userActionEarnedDays ) {
  ...
  if ( userActionEarnedDays ) {
    daysLimit += userActionEarnedDays;
  }
