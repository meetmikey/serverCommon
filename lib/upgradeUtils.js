...
exports.updateUserAccountStatus = function( user, forceResumeDownloadJob, callback ) {
	...
	winston.doInfo('updateUserAccountStatus', {user: user, forceResumeDownloadJob: forceResumeDownloadJob});
		...
		if ( err ) {
			...
			upgradeUtils.updateUserDaysLimit( user, function(err, daysLimitChanged) {
				if ( err ) {
					...
					winston.doInfo('updateUserAccountStatus, creating resume download job', {user: user, daysLimitChanged: daysLimitChanged});
...
exports.calculateDaysLimit = function( user, foundReferrals, foundExtraDays ) {
  ...
  winston.doInfo('calculateDaysLimit', {daysLimit: daysLimit, user: user, foundReferrals: foundReferrals, foundExtraDays: foundExtraDays});
  return daysLimit;
}
...
exports.creditChromeStoreReview = function( userEmail, callback ) {
	...
	UserModel.findOne( {email: userEmail}, function(mongoErr, user) {
		...
		} else if ( user.clickedChromeStoreReview ) {
			winston.doWarn('user already clicked chromestore review', {userId: user._id});
			callback();

		} else {
			user.clickedChromeStoreReview = true;
			var updateData = {$set: {
		  	clickedChromeStoreReview: true
	  	}};
