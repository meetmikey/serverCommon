...
exports.uploadResponseToS3 = function(response, s3Path, callback) {
  s3Utils.uploadResponseToS3Attempt( response, s3Path, 0, callback );
}

exports.uploadResponseToS3Attempt = function(response, s3Path, attempts, callback) {
  ...
  s3Utils.client.putStream( response, s3Path, headers, function(s3Err, s3Response) {
    ...
    if ( clientPutStreamHasCalledBack ) {
      ...
      if ( s3Err ) {
        var message = 'uploadResponseToS3 failed'
        winston.doWarn( message, {s3Path: s3Path});
        s3Utils.retryUploadResponseToS3( message, response, s3Path, attempts, callback );

      } else if (s3Response.statusCode !== 200) {
        var message = 'putStream non 200 status code';
...
exports.retryUploadResponseToS3 = function( errorMessage, response, s3Path, attempts, callback ) {

  if ( attempts < constants.S3_RETRIES ) {
    winston.doInfo('Retrying uploadToS3 for file: ', {s3Path : s3Path, attempts : attempts});
    s3Utils.uploadResponseToS3Attempt( response, s3Path, attempts + 1, callback );
