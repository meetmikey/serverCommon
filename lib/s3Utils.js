...
exports.getAttachmentS3Path = function(attachmentId, userId) {
  
  if ( ! attachmentId ) { winston.warn('s3Utils: getAttachmentS3Path: no attachmentId'); }
  if ( ! userId ) {
    winston.warn('s3Utils: getAttachmentS3Path: no userId');
    userId = 'MISSING_USER_ID';
  }

  var s3Path = conf.aws.s3Folders.attachments + '/' + userId + '/' + attachmentId;
  return s3Path;
}
...
exports.getFile = function(s3Path, callback) {
