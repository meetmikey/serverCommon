...
exports.putGzipBuffer = function( buffer, s3Path, headers, callback ) {
  ...
  zlib.gzip( buffer, function( zlibError, gzipBuffer ) {
    ...
    if ( zlibError ) {
      ...
      s3Utils.putBuffer( gzipBuffer, s3Path, headers, 0, callback );
    }
  });
}

exports.putBuffer = function( buffer, s3Path, headers, attempts, callback ) {
  ...
  winston.doInfo('about to putBuffer', {s3Path: s3Path, size: buffer.length});

  s3Utils.client.putBuffer( buffer, s3Path, headers, function(err, res) {
    if ( err ) {
      winston.doWarn ('Error attempting upload to s3', {err : err});
      s3Utils.retryPutBuffer(err, buffer, s3Path, headers, useGzip, attempts, callback);
      ...
      winston.doWarn ('s3Utils: putBuffer: bad response or non-200 statusCode', warnData);
      s3Utils.retryPutBuffer('Error: bad response or non-200 statusCode: '  + statusCode, buffer, s3Path, headers, attempts, callback);
...
exports.retryPutBuffer = function( err, buffer, s3Path, headers, attempts, callback ) {

  var stacktrace = new Error().stack;
  winston.doInfo ('retry called', {s3Path : s3Path, attempts : attempts, stacktrace: stacktrace});

  if ( attempts < constants.S3_RETRIES ) {
    winston.doInfo('Retrying upload for file: ', {s3Path : s3Path, attempts : attempts});
    s3Utils.putBuffer( buffer, s3Path, headers, attempts + 1, callback );
    ...
    callback( winston.makeS3Error('Max upload attempts exceeded for file', {finalError : err, 'intendedPath' : s3Path}) );
  }
}
