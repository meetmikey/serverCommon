...
exports.putBuffer = function(buffer, s3Path, headers, useGzip, attempts, callback) {
  ...
  if ( useGzip ) {
      ...
      s3Utils.client.putBuffer(gzipBuffer, s3Path, headers,
        function(err, res) {
          if ( err ) {
            ...
            s3Utils.retry(err, buffer, s3Path, headers, useGzip, attempts, callback);
          } 
          else if (res && res.statusCode !== 200) {
            s3Utils.printAWSResponse (res);
            winston.doWarn ('non 200 status code', {'statusCode' : res.statusCode, 's3Path' : s3Path});
            s3Utils.retry('Error: non 200 status code for upload: '  + res.statusCode, buffer, s3Path, headers, useGzip, attempts, callback);
          }
          else {
    ...
    var startTime = Date.now();
    winston.doInfo('about to putBuffer (not gzipped)', {s3Path: s3Path, size: buffer.length});
    ...
    s3Utils.client.putBuffer(buffer, s3Path, headers,
      function(err, res) {
        if ( err ) {
          ...
          winston.doInfo('completed putBuffer (not gzipped)', {s3Path: s3Path, size: buffer.length, elapsedTime: elapsedTime, metric: metric});
...
// TODO: don't re-zip if we've already zipped...
exports.retry = function (err, buffer, s3Path, headers, useGzip, attempts, callback) {
  ...
  if (attempts < constants.S3_RETRIES) {
    ...
    callback( winston.makeS3Error('Max upload attempts exceeded for file' , 
      {finalError : err, 'intendedPath' : s3Path}) );
