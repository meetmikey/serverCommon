...
exports.signedURL = function(s3Path, expiresMinutesInput, model) {
  ...
  if (model.hash) {
    ...
    console.log ('signedUrl for model', model);
...
exports.downloadAndSaveStaticHTTPSImage = function(url, s3Path, callback) {
  ...
  https.get( options, function( response ) {
    ...
    s3Utils.uploadResponseToS3( response, s3Path, headers, callback );
...
exports.uploadResponseToS3 = function(response, s3Path, headers, callback) {
  s3Utils.uploadResponseToS3Attempt( response, s3Path, headers, 0, callback );
}

exports.uploadResponseToS3Attempt = function(response, s3Path, headers, attempts, callback) {
  ...
  s3Utils.client.putStream( response, s3Path, headers, function(s3Err, s3Response) {
    ...
    if ( clientPutStreamHasCalledBack ) {
      ...
      if ( s3Err ) {
        var message = 'uploadResponseToS3 failed: ' + s3Err.message;
        winston.doWarn( message, {s3Path: s3Path});
        s3Utils.retryUploadResponseToS3( message, response, s3Path, headers, attempts, callback );
...
exports.retryUploadResponseToS3 = function( errorMessage, response, s3Path, headers, attempts, callback ) {

  if ( attempts < constants.S3_RETRIES ) {
    winston.doInfo('Retrying uploadToS3 for file: ', {s3Path : s3Path, attempts : attempts});
    s3Utils.uploadResponseToS3Attempt( response, s3Path, headers, attempts + 1, callback );
...
exports.putStream = function (stream, s3Path, headers, useGzip, callback) {
  ...
  s3Utils.uploadResponseToS3 (stream, s3Path, headers, callback);
}
