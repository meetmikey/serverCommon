...
var workerReindexQueue = aws.createSQSClient(conf.aws.key, conf.aws.secret, workerReindexOptions);
...
exports.pollWorkerReindexQueue = function( handleMessage, maxWorkers, workerTimeout ) {
  sqsConnect.pollQueue( workerQueue, QUEUE_NAME_WORKER_REINDEX, handleMessage, maxWorkers, workerTimeout );
}
...
exports.getMessageFromQueue = function( queue, queueName, callback ) {
  ...
  queue.call( "ReceiveMessage", 
    ...
    if ( sqsError ) {
      callback( winston.makeError( 'sqs error from ReceiveMessage', {queueName: queueName, sqsError: sqsError} ) );
      ...
      if (tooManyDequeues) {
        console.log ('tooManyDequeues', tooManyDequeues);
        ...
        fail.save (function (err) {
          ...
          if (err) {
            winston.doError ('Error saving failed queue job', {err : err});
            return;
          } else {
            // delete the message
      ...
      callback( null, messageBody, function( err ) {
        ...
        if ( err ) {
          ...
          var flag = winston.getDeleteFromQueueFlag (err);
...
exports.updateWorkerLastContactTime = function( workerId, queue, queueName, maxWorkers, handleMessageFunction ) {
