...
exports.pollQueue = function(queue, handleMessage, maxConcurrentConnections) {

  //console.log('pollQueue running...');

  sqsConnect.getMessageFromQueue(queue, function(err, message, deleteMessageCallback) {
    if ( err ) {
      console.error('Error: pollQueue: received error from getMessageFromQueue: ' + err);
      deleteMessageCallback(err);
    } else {
...
exports.pollMailDownloadQueue = function( handleMessage, maxConcurrentConnections ) {
  this.pollQueue( mailDownloadQueue, handleMessage, maxConcurrentConnections );
}