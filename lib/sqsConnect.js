...
exports.checkWorkers = function( queue, queueName, handleMessageFunction, maxWorkers, workerTimeout ) {
  ...
  if ( ! workers[queueName] )  {
    winston.doError('No worker queue!', {queueName: queueName, workerId: workerId});
    ...
    for ( var workerId in workers[queueName] ) {
      ...
      if ( elapsedTime > workerTimeout ) {
        winston.doError('worker timed out! deleting worker.', {queueName: queueName, workerId: workerId, elapsedTime: elapsedTime});
