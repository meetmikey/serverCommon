...
var stopWorkForQueueReceived = {
  Worker : false,
  WorkerQuick : false,
  mailDownload : false,
  mailReader : false,
  mailReaderQuick : false,
  mailActiveConnection : false,
  CacheInvalidation : false,
  WorkerReindex : false,
  ThumbnailQuick : false,
  Thumbnail : false
};

var allWorkersDoneMessage = 'All workers done.';

exports.stopSignal = function() {
  stopSignalReceived = true;
  conf.turnDebugModeOn();
  winston.doInfo('SQS: Received stop signal');
  if ( sqsConnect.getNumTotalWorkers() == 0 ) {
    winston.doInfo( allWorkersDoneMessage );
  }
}

exports.setStopSignalForQueue = function (queueName, reason) {
  if (!stopWorkForQueueReceived [queueName]) {
    stopWorkForQueueReceived [queueName] = true;
    conf.turnDebugModeOn();
    winston.doInfo('SQS: Received stop signal for queue', {name : queueName});
    var msg = 'Stop signal for queue: ' + queueName + ' on host: ' + os.hostname();

    sesUtils.sendInternalNotificationEmail (msg, queueName + ' on ' + os.hostname() + ' stopped work due to ' + reason, function (err) {});
  }
}
...
exports.addMessageToMailReaderQueue = function( message, callback ) {
  sqsConnect.addMessageToQueue( mailReaderQueue, QUEUE_NAME_MAIL_READER, message, 0, callback );
}
...
exports.addMessageToMailReaderQuickQueue = function( message, callback ) {
  sqsConnect.addMessageToQueue( mailReaderQuickQueue, QUEUE_NAME_MAIL_READER_QUICK, message, 0, callback );
}
...
exports.addMessageToMailDownloadQueue = function( message, delayOrCallback, callback ) {
  //TODO: remove backwards compatibility
  if (callback) {
    sqsConnect.addMessageToQueue( mailDownloadQueue, QUEUE_NAME_MAIL_DOWNLOAD, message, delayOrCallback, callback );
  } else {
    sqsConnect.addMessageToQueue( mailDownloadQueue, QUEUE_NAME_MAIL_DOWNLOAD, message, 0, delayOrCallback );
  }
}
...
exports.addMessageToMailActiveConnectionQueue = function( message, callback ) {
  sqsConnect.addMessageToQueue( mailActiveConnectionQueue, QUEUE_NAME_MAIL_ACTIVE_CONNECTION, message, 0, callback );
}
...
/*
  ...
  --------------------------------------
*/
var QUEUE_NAME_THUMBNAIL = 'Thumbnail';
var thumbnailOptions = {
  "path" : '/' + awsAccountId + '/' + conf.aws.sqsThumbnailQueue
}
var thumbnailQueue = aws.createSQSClient( awsKey, awsSecret, thumbnailOptions );
exports.addMessageToThumbnailQueue = function( message, callback ) {
  sqsConnect.addMessageToQueue( thumbnailQueue, QUEUE_NAME_THUMBNAIL, message, 0, callback );
}

exports.pollThumbnailQueue = function ( handleMessage, maxWorkers, workerTimeout ) {
  sqsConnect.pollQueue( thumbnailQueue, QUEUE_NAME_THUMBNAIL, handleMessage, maxWorkers, workerTimeout );
}
workers[QUEUE_NAME_THUMBNAIL] = {};


/*
  ThumbnailQuick Queue
  --------------------------------------
*/
var QUEUE_NAME_THUMBNAIL_QUICK = 'ThumbnailQuick';
var thumbnailQuickOptions = {
  "path" : '/' + awsAccountId + '/' + conf.aws.sqsThumbnailQuickQueue
}
var thumbnailQuickQueue = aws.createSQSClient( awsKey, awsSecret, thumbnailQuickOptions );
exports.addMessageToThumbnailQuickQueue = function( message, callback ) {
  sqsConnect.addMessageToQueue( thumbnailQuickQueue, QUEUE_NAME_THUMBNAIL_QUICK, message, 0, callback );
}

exports.pollThumbnailQuickQueue = function ( handleMessage, maxWorkers, workerTimeout ) {
  sqsConnect.pollQueue( thumbnailQuickQueue, QUEUE_NAME_THUMBNAIL_QUICK, handleMessage, maxWorkers, workerTimeout );
}
workers[QUEUE_NAME_THUMBNAIL_QUICK] = {};
...
exports.addMessageToWorkerQueue = function( message, callback ) {
  sqsConnect.addMessageToQueue( workerQueue, QUEUE_NAME_WORKER, message, 0, callback );
}
...
/*
  ...
  --------------------------------------
*/
var QUEUE_NAME_WORKER_QUICK = 'WorkerQuick';
var workerQuickOptions = {
  "path" : '/' + awsAccountId + '/' + conf.aws.sqsWorkerQuickQueue
}
var workerQuickQueue = aws.createSQSClient( awsKey, awsSecret, workerQuickOptions);
exports.addMessageToWorkerQuickQueue = function( message, callback ) {
  sqsConnect.addMessageToQueue( workerQuickQueue, QUEUE_NAME_WORKER_QUICK, message, 0, callback );
}

exports.pollWorkerQuickQueue = function ( handleMessage, maxWorkers, workerTimeout ) {
  sqsConnect.pollQueue( workerQuickQueue, QUEUE_NAME_WORKER_QUICK, handleMessage, maxWorkers, workerTimeout );
}
workers[QUEUE_NAME_WORKER_QUICK] = {};
...
exports.addMessageToWorkerReindexQueue = function( message, callback ) {
  sqsConnect.addMessageToQueue( workerReindexQueue, QUEUE_NAME_WORKER_REINDEX, message, 0, callback );
}
...
exports.addMessageToCacheInvalidationQueue = function( message, callback ) {
  sqsConnect.addMessageToQueue( cacheInvalidationQueue, QUEUE_NAME_CACHE_INVALIDATION, message, 0, callback );
}
...
exports.pollQueue = function( queue, queueName, handleMessageFunction, maxWorkersInput, workerTimeoutInput ) {
  ...
  if ( checkWorkersIntervals[queueName] ) {
    ...
    checkWorkersIntervals[queueName] = setInterval( function() {
      if ( !( stopSignalReceived || stopWorkForQueueReceived[queueName] ) || sqsConnect.getNumTotalWorkers() > 0 ) {
...
exports.addMessageToQueue = function( queue, queueName, message, delaySeconds, callback ) {
  utils.runWithRetries( sqsConnect.addMessageToQueueNoRetry, constants.SQS_RETRIES, callback, queue, queueName, message, delaySeconds );
}


exports.addMessageToQueueNoRetry = function( queue, queueName, message, delaySeconds, callback ) {
  ...
  queue.call( 'SendMessage', sqsMessage, function( sqsError, result ) {
    ...
    if ( callback ) {
      ...
      if ( sqsError ) {
        var sqsErrorMessage = sqsError.toString();
...
exports.workQueue = function( workerId, queue, queueName, maxWorkers, handleMessageFunction, previousConsecutiveMisses ) {
  ...
  if ( stopSignalReceived || stopWorkForQueueReceived[queueName] ) {
...
exports.checkWorkers = function( queue, queueName, handleMessageFunction, maxWorkers, workerTimeout ) {
  ...
  if ( ! workers[queueName] )  {
    ...
    if ( !( stopSignalReceived || stopWorkForQueueReceived[queueName] ) && (numWorkers < maxWorkers)) {
